...
class Veckor extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		...
		axios
			...
			.then(res => {
							...
							if (Array.isArray(obj[key])) {
								for (var x = obj[key].length - 1; x >= 0; x--) {
									if (obj[key][x].person == clicked_user) {
										...
										//new_user_list[arry[i].project_namn].push([key])
										//console.log(new_user_list[arry[i].project_namn]);
										//new_user_list[arry[i].project_namn][key].push(obj[key][x].timmar);
				...
				//this.setState({ new_user_list: new_user_list });
