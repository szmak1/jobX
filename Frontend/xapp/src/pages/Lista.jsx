...
class Lista extends Component {
	...
	componentDidMount() {
		...
		axios.get("http://xapp.tst/wp-json/wp/v2/acf/options", {
					headers: {
			...
			<div className="Medarbetare-grid">
				...
				<Anstallda>
					...
					{this.state.medarbetare.map(function(post) {
							//console.log(post);
							return (
								<div className="card" key={post.medarbetare_id} onClick={(e) => that.goToMedarbetare(post.medarbetare_id,post.medarbetare_bild,e)}>
										...
										<div className="container2" >
											<h4>{post.medarbetare_name}</h4>
											<img src={post.medarbetare_bild} alt="" className="medarbetare_bild"/>
											<h3>{post.medarbetare_titel}</h3>
										</div>
